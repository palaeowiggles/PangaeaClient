...
public struct PangaeaClient {
    ...
    private func fetch(pangaeaID inputID: String) -> Promise<PangaeaMeta> {
		...
		return Promise { seal in
			...
			let metaURL = URL(string: pangaeaID+"?format=metainfo_xml", relativeTo: configuration.metaBaseURL)!
			let dataURL = URL(string: "\(configuration.ddsBaseURL)\(configuration.doiBase)\(pangaeaID)")!
			
			let fetchMetaDataFunc = URLSession.shared.dataTask(.promise, with: metaURL).compactMap{
                String(data: $0.data, encoding: .utf8)
            }
            
            var fetchMetaData : Promise<String> {
                get {
                    if let cachedPangaeaMeta = try? pangaeaMetaCacheController?.get(id: pangaeaID) {
            ...
            var fetchData : Promise<String?> {
                get {
                    if let cachedPangaeaData = try? pangaeaDataCacheController?.get(id: pangaeaID) {
			...
			fetchMetaData.done { metaDataStr in
                let _ = try? self.pangaeaMetaCacheController?.set(id: pangaeaID, data: metaDataStr)
                
				if let metaData = metaDataStr.data(using: .utf8) {
					let meta = try self.decoder.decode(PangaeaMetaBase.self, from: metaData)
					...
					guard //let hierarchyLevelStr = meta.technicalInfo?.entries[PMTechnicalInfo2.ValueCodingKeys.hierarchyLevel.rawValue],
						...
						let mimeTypeStr = meta.technicalInfo?.entries[PMTechnicalInfo2.ValueCodingKeys.mimeType.rawValue],
						let mimeType = PMTechnicalInfoMimeType(rawValue: mimeTypeStr),
						let accessLevelStr = meta.technicalInfo?.entries[PMTechnicalInfo2.ValueCodingKeys.accessLevel.rawValue],
						let accessLevel = PMTechnicalInfoAccessLevel(rawValue: accessLevelStr)
						
						else {
							seal.reject(PangaeaError.dataCorrupted(message: "cannot determine hierarchyLevel, mimeType, accessLevel from metaInfo \(String(describing: meta.technicalInfo?.entries))"))
							return
					...
					guard meta.matrixColumns.count  > 0  else {
						...
						seal.fulfill(PangaeaMeta.dataParent(meta: meta, childrenURLs: children ?? []))
						return
					}
					...
					fetchData.done { dataStr in
                        if let dataStr = dataStr {
                            let _ = try? self.pangaeaDataCacheController?.set(id: pangaeaID, data: dataStr)
                        }
						//print(dataStr)
						if let data = dataStr {
							...
							guard meta.matrixColumns.count == splitData.first?.count else {
								seal.reject(PangaeaError.dataCorrupted(message: "meta column count different from first data row"))
								return
							}
							...
							for colIndex in 0..<meta.matrixColumns.count {
								...
								case .numeric:
									...
									let dataColStruct = PangaeaResultColumn(caption: col.Caption, data: DataColumn.numeric(dataVal), dataMeta: col)
									dataCols.append(dataColStruct)
								case .string:
									let dataVal = columns[colIndex]
									let dataColStruct = PangaeaResultColumn(caption: col.Caption, data: DataColumn.text(dataVal), dataMeta: col)
									dataCols.append(dataColStruct)
								case .geocode:
									let dataVal = columns[colIndex].map{Double($0) ?? Double.nan}
									let dataColStruct = PangaeaResultColumn(caption: col.Caption, data: DataColumn.geocode(dataVal), dataMeta: col)
									dataCols.append(dataColStruct)
								case .datetime:
									let dataVal = columns[colIndex].map{dateFormatterISO.date(from:$0)}
									let dataColStruct = PangaeaResultColumn(caption: col.Caption, data: DataColumn.datetime(dataVal), dataMeta: col)
									dataCols.append(dataColStruct)
								case .uri:
									let dataVal = columns[colIndex].compactMap{URL(string: $0)}
									let dataColStruct = PangaeaResultColumn(caption: col.Caption, data: DataColumn.uri(dataVal), dataMeta: col)
									dataCols.append(dataColStruct)
								}
							...
							seal.fulfill(PangaeaMeta.dataset(meta: meta, data: dataCols, rowCount: rowCount))
						}
					}
