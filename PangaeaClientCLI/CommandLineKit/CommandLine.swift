...
public class CommandLine {
  ...
  public func printUsage<TargetStream: TextOutputStream>(_ to: inout TargetStream) {
    ...
    /**
   * Prints a custom usage message.
   *
   * - parameter to: An OutputStreamType to write the error message to.
   */
  public func printCustomUsage<TargetStream: TextOutputStream>(_ str: String, _ to: inout TargetStream) {
    /* Nil coalescing operator (??) doesn't work on closures :( */
    let format = formatOutput != nil ? formatOutput! : defaultFormat

    let name = URL(fileURLWithPath: _arguments[0]).lastPathComponent
    print(format(str, .about), terminator: "", to: &to)
  ...
  public func printCustomUsage(_ str: String){
	var out = StderrOutputStream.stream
    printCustomUsage(str, &out)
  }
}
